...
interface AddStudySessionModalProps {
  open: boolean;
  onClose: () => void;
  onAdd: (session: Omit<StudySession, 'id' | 'createdAt'>) => void;
}

export default function AddStudySessionModal({ open, onClose, onAdd }: AddStudySessionModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const filteredLessons = lessons.filter(l => l.trim()).map(name => ({
      name: name.trim(),
      completed: false
    }));
